...
class Game extends React.Component {
    constructor(props) { //boilerplate class component syntax
      ...
      this.state = { // set state to include history -- an array of objects, each object contains an array of the board state at a certain point in the game
        ...
        xIsNext: true, // set state to determine who's turn it is
      };
    }
    ...
    render() {
      ...
      const moves = history.map((step, move) => {
        const desc = move ? // ternary, if we're at the 0 indice that means we have no board history thus the button being rendered should be to go to the start, if move is > 0 is then it will say to go to that move in the board's history.
          'Go to move #' + move :
          'Go to game start';
        return (
          <li>
